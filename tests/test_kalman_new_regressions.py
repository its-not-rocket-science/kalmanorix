...
def test_correlation_aware_fuser_rejects_unknown_mode() -> None:
    profile = ResidualCorrelationProfile(module_names=["a"], correlation_matrix=np.eye(1))
...
def test_correlation_aware_fuser_requires_profile_modules_to_exist() -> None:
    profile = ResidualCorrelationProfile(module_names=["a"], correlation_matrix=np.eye(1))
    fuser = CorrelationAwareKalmanFuser(correlation_profile=profile, mode="effective_sample_size")
...
def test_uncertainty_factory_whitespace_case_and_stochastic_pass_floor() -> None:
    method = create_uncertainty_method(
        config=UncertaintyMethodConfig(method="  EMBEDDING_NORM_SIGMA2  ", base_sigma2=0.7),
        embed=_toy_embed,
    )
    ...
    sigma2 = StochasticForwardSigma2(embed_stochastic=stochastic_embed, n_passes=1, base_sigma2=0.3)
    value = sigma2("tiny")
    assert value > 0.0
...
def test_uncertainty_calibration_status_contract_for_powered_vs_underpowered(tmp_path: Path) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"benchmark", "methods", "paired_statistics", "decision", "bucket_analysis"},
        ),
        (
            "uncertainty_calibration",
            "uncertainty_calibration/summary.json",
            {"selected_objective", "objective_reports", "selected_report", "selection_is_validation_only"},
        ),
        (
        ...
        (
            ...
            {"assumptions", "single_query_strategies", "batch_strategies", "numerical_deviation_vs_legacy", "canonical_rerun"},
        ),
    ],
)
def test_major_results_artifact_schema_contracts(name: str, summary_relpath: str, required_keys: set[str]) -> None:
    summary_path = Path("results") / summary_relpath
    assert summary_path.exists(), f"Expected committed artifact for {name}: {summary_path}"
...
def test_snapshot_uncertainty_calibration_markdown(tmp_path: Path) -> None:
    ...
    expected = Path("tests/snapshots/uncertainty_calibration_report_expected.md").read_text(encoding="utf-8")
...
def test_snapshot_correlation_aware_fusion_markdown(tmp_path: Path) -> None:
    ...
    report = (out / "report.md").read_text(encoding="utf-8")
    expected = Path("tests/snapshots/correlation_aware_fusion_report_expected.md").read_text(encoding="utf-8")
    assert report == expected


def test_optimized_kalman_numerical_equivalence_tight_tolerance() -> None:
    modules = [
        SEF(name="m0", embed=lambda q: np.array([1.0, 0.1, len(q) * 0.001]), sigma2=1e-4),
        SEF(name="m1", embed=lambda q: np.array([0.9, 0.2, len(q) * 0.0015]), sigma2=2e-4),
        SEF(name="m2", embed=lambda q: np.array([0.2, 0.95, len(q) * 0.0003]), sigma2=8e-4),
    ]
    ...
    for xl, xo, wl, wo, ml, mo in zip(x_legacy, x_opt, w_legacy, w_opt, m_legacy, m_opt):
        assert np.allclose(xo, xl, rtol=1e-11, atol=1e-12)
        assert np.allclose(mo["fused_covariance"], ml["fused_covariance"], rtol=1e-11, atol=1e-12)
        for k in wl:
