...
Specialist retrieval evaluation sits at the intersection of three active tooling areas: embedding models, retrieval benchmarking, and statistical evaluation. SentenceTransformers [@reimers2019sentencebert] and Hugging Face Transformers provide embedding backends; BEIR [@thakur2021beir] defines cross-domain retrieval benchmarks; FAISS [@johnson2017faiss] handles efficient vector search. For statistical evaluation, the IR community relies on guidance from Smucker et al. [-@smucker2007significance] and Demšar [-@demsar2006statistical] on paired significance testing and multi-dataset comparison.

No existing tool integrates specialist routing, fusion-strategy comparison, uncertainty-aware weighting, and claim-gated reporting into one pipeline. Kalmanorix fills that gap without replacing the underlying libraries: it wraps embedding backends as SEF specialists, delegates vector search to FAISS-compatible indices, uses BEIR-style query sets, and adds routing, fusion, and evidence-export on top.

The claim-gating component addresses a distinct gap in tooling: IR evaluation literature identifies risks of underpowered and uncorrected comparisons [@smucker2007significance; @dror2017replicability], but software that enforces those practices programmatically—writing machine-readable verdicts and preserving negative outcomes as first-class artifacts—does not exist as a standalone reusable package.
...
Kalmanorix is the operative pipeline for two companion peer-submitted manuscripts:

- A TMLR methodology submission on claim-gated evaluation protocol for small retrieval deltas.
- An arXiv empirical report documenting a powered null result ($n=1{,}193$ query pairs, Holm-adjusted $p=1.0$) for Kalman-style uncertainty weighting over mean fusion on a mixed-domain benchmark.

Both manuscripts source headline metrics and baseline tables directly from artifacts generated by Kalmanorix scripts (`results/evidence_registry.json`, `results/claim_gate.json`), making the software the operative pipeline for peer-submitted scientific claims rather than a standalone demonstration.

Near-term significance beyond the companion papers: the claim-gating template and paired inference protocol are designed to be reusable across retrieval studies facing overclaim risk from small metric deltas. The MIT license, Zenodo archive ([10.5281/zenodo.20318011](https://doi.org/10.5281/zenodo.20318011)), CI test suite, and structured artifact outputs lower the adoption barrier for future studies that need disciplined null-result documentation.
